...
impl Layout {
    pub fn validate(&mut self) {
        if self.items.len() > 0 {
            ...
            if remaining_free_layout_space == 0 {
                ...
                let item = self.items.remove(item_with_lowest_current_max_diff);
    ...
    fn get_item_with_lowest_current_max_diff(&self) -> usize {
        let mut idx = 0;
    ...
    fn get_item_with_highest_current_vs_expected_stretch_diff(&self) -> usize {
        ...
        for item in &self.items {
            ...
            let diff = expected_stretch - current_stretch;
